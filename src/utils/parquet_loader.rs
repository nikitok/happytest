...
impl ParquetDataSource {
    ...
    fn init_reader(&mut self) -> Result<()> {
        ...
        let file = File::open(&self.file_path).map_err(|e| {
            TradeError::DataLoadingError(format!("Failed to open parquet file: {}", e))
        })?;

        let builder = ParquetRecordBatchReaderBuilder::try_new(file).map_err(|e| {
            TradeError::DataLoadingError(format!("Failed to create parquet reader: {}", e))
        })?;
        ...
        let batch_reader = builder.with_batch_size(10000).build().map_err(|e| {
            TradeError::DataLoadingError(format!("Failed to build parquet reader: {}", e))
        })?;
    ...
    fn load_next_batch(&mut self) -> Result<bool> {
        ...
        match reader.next() {
            ...
            Some(Err(e)) => Err(TradeError::DataLoadingError(format!(
    ...
    fn parse_row(&self, batch: &RecordBatch, row_idx: usize) -> Result<OrderBook> {
        ...
        let ts_column = batch
            ...
            .ok_or_else(|| {
                TradeError::DataLoadingError("Missing 'timestamp' column in parquet".to_string())
            })?
            .as_any()
            .downcast_ref::<Int64Array>()
            .ok_or_else(|| {
                TradeError::DataLoadingError("'timestamp' column is not Int64".to_string())
            })?;

        let bids_column = batch
            .column_by_name("bids")
            .ok_or_else(|| {
                TradeError::DataLoadingError("Missing 'bids' column in parquet".to_string())
            })?
            .as_any()
            .downcast_ref::<StringArray>()
            .ok_or_else(|| {
                TradeError::DataLoadingError("'bids' column is not String".to_string())
            })?;

        let asks_column = batch
            .column_by_name("asks")
            .ok_or_else(|| {
                TradeError::DataLoadingError("Missing 'asks' column in parquet".to_string())
            })?
            .as_any()
            .downcast_ref::<StringArray>()
            .ok_or_else(|| {
                TradeError::DataLoadingError("'asks' column is not String".to_string())
            })?;
        ...
        let bid_array: Vec<Vec<String>> = serde_json::from_str(bids_json).map_err(|e| {
            TradeError::DataLoadingError(format!("Failed to parse bids JSON: {}", e))
        })?;

        let ask_array: Vec<Vec<String>> = serde_json::from_str(asks_json).map_err(|e| {
            TradeError::DataLoadingError(format!("Failed to parse asks JSON: {}", e))
        })?;
