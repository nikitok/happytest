...
pub fn validate_config(config: &AppConfig) -> Result<()> {
    ...
    if config.backtest.fill_rate < 0.0 || config.backtest.fill_rate > 1.0 {
        return Err(TradeError::InvalidTradeParameters(format!(
            "Fill rate must be between 0.0 and 1.0, got {}",
            config.backtest.fill_rate
        )));
    }

    if config.backtest.rejection_rate < 0.0 || config.backtest.rejection_rate > 1.0 {
        return Err(TradeError::InvalidTradeParameters(format!(
            "Rejection rate must be between 0.0 and 1.0, got {}",
            config.backtest.rejection_rate
        )));
    }

    if config.backtest.margin_rate < 0.0 || config.backtest.margin_rate > 1.0 {
        return Err(TradeError::InvalidTradeParameters(format!(
            "Margin rate must be between 0.0 and 1.0, got {}",
            config.backtest.margin_rate
        )));
    }
