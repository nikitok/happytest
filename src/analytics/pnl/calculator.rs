...
impl PnlReport {
    ...
    pub fn graph_equity(
        ...
        for (symbol, symbol_trades) in trades_by_symbol {
            let metrics = self.calculate_equity_metrics(&symbol_trades, method, initial_capital, margin_rate);
            ...
            let max_equity = metrics.peak_equity.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
            let equity_padding = (max_equity - min_equity) * 0.1;

            let mut upper_chart = ChartBuilder::on(&upper)
                .caption(
                    format!("{} - Equity Curve (Initial: ${:.0})", symbol, initial_capital),
            ...
            let peak_data: Vec<(i64, f64)> = metrics.timestamps.iter()
                .cloned()
                .zip(metrics.peak_equity.iter().cloned())
                .collect();
            upper_chart.draw_series(LineSeries::new(peak_data, &GREEN.mix(0.5)))?
                .label("Peak Equity")
                .legend(|(x, y)| PathElement::new(vec![(x, y), (x + 15, y)], &GREEN.mix(0.5)));

            // Draw equity curve
            let equity_data: Vec<(i64, f64)> = metrics.timestamps.iter()
                .cloned()
                .zip(metrics.equity.iter().cloned())
                .collect();
            upper_chart.draw_series(LineSeries::new(equity_data.clone(), &BLUE))?
                .label("Equity")
            ...
            if let Some((last_time, last_equity)) = equity_data.last() {
                let color = if *last_equity >= initial_capital { &GREEN } else { &RED };
            ...
            let dd_data: Vec<(i64, f64)> = metrics.timestamps.iter()
                .cloned()
                .zip(metrics.drawdown.iter().cloned())
                .collect();
            lower_chart.draw_series(AreaSeries::new(
                dd_data.clone(),
                0.0,
                &RED.mix(0.3),
            ))?
                .label(format!("Drawdown (Max: ${:.2})", max_dd))
                .legend(|(x, y)| Rectangle::new([(x, y - 5), (x + 15, y + 5)], RED.mix(0.3).filled()));

            // Draw margin used
            let margin_data: Vec<(i64, f64)> = metrics.timestamps.iter()
                .cloned()
                .zip(metrics.margin_used.iter().cloned())
                .collect();
            lower_chart.draw_series(LineSeries::new(margin_data, &MAGENTA))?
            ...
            println!("  Recommended Capital: ${:.2} (margin + drawdown buffer)", required_capital);
