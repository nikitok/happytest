...
use super::executor::{BacktestConfig, BacktestTradeEmitter, TradeEmitter};
use crate::core::{DataSource, Result, TradeError, TradeState};
use crate::strategy::{GptMarketMaker, GptMarketMakerConfig, Strategy};
use crate::utils::{extract_symbol_from_filename, FileDataSource, ParquetDataSource};
...
impl BacktestEngine {
    ...
    pub fn run_backtest(&self, data_file: &Path, strategy_name: &str) -> Result<TradeState> {
        ...
        let mut strategy: Box<dyn Strategy> = match strategy_name {
            ...
            _ => {
                return Err(TradeError::InvalidTradeParameters(format!(
                    "Unknown strategy: {}",
                    strategy_name
                )))
            }
        ...
        let mut data_source: Box<dyn DataSource> =
            if data_file.extension().and_then(|s| s.to_str()) == Some("parquet") {
        ...
        info!(
            "Running backtest for {} with {} orderbook messages",
            symbol, total_messages
        );
        ...
        println!(
            "Backtest completed in {:.2} seconds",
    ...
    pub fn run_backtest_with_custom_strategy(
        ...
        let execution_time = start_time.elapsed();
        info!(
            "Backtest completed in {:.2} seconds ({} messages processed)",
            execution_time.as_secs_f64(),
            processed
        );
