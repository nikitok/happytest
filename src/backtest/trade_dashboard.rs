use crate::core::{CapitalMetrics, ClosedTrade, OrderBook, PnLResult, Trade, TradeState};
...
impl TradeDashboard {
    ...
    pub fn pnl(&mut self, symbol: &str) -> HashMap<String, PnLResult> {
        ...
        info!(
            "Calculating PNL for {} with {} trades",
            symbol,
            trades.len()
        );
    ...
    fn process_trades(&self, trades: &[&Trade], symbol: &str) -> PnLResult {
        ...
        for trade in trades {
            ...
            let pos_list = positions.entry(trade.symbol.clone()).or_default();
    ...
    pub fn calculate_trading_costs(&self, _symbol: &str) -> HashMap<&str, f64> {
        ...
        let buy_trades = trades
            .iter()
            .filter(|t| t.side.to_uppercase() == "BUY")
            .count();
        let sell_trades = trades
            .iter()
            .filter(|t| t.side.to_uppercase() == "SELL")
            .count();
    ...
    pub fn recalculate_capital_metrics(&mut self, symbol: &str) {
        ...
        for trade in all_trades {
            ...
            while orderbook_idx < orderbooks.len()
                && orderbooks[orderbook_idx].current_time < trade.time
            {
                orderbook_idx += 1;
            }
            ...
            let new_avg_price = if quantity == 0.0 {
                trade.price
            } else if (quantity > 0.0 && trade_quantity > 0.0)
                || (quantity < 0.0 && trade_quantity < 0.0)
            {
            ...
            self.calculate_capital_metrics(
                trade.time,
                &HashMap::from([(symbol.to_string(), current_price)]),
            );
        }
    }
    ...
    fn calculate_capital_metrics(&mut self, timestamp: i64, current_prices: &HashMap<String, f64>) {
        ...
        let required_capital =
            total_margin_requirement + 0.0_f64.max(-total_unrealized_pnl) + safety_buffer;
        ...
        self.open_positions_value_history
            .push(total_open_positions_value);
    }
    ...
    pub fn get_capital_metrics(&mut self, symbol: &str) -> CapitalMetrics {
        ...
        let max_open_positions_value = self
            .open_positions_value_history
            .iter()
            .fold(0.0_f64, |a, &b| a.max(b));
        let average_capital_utilization =
            self.capital_history.iter().sum::<f64>() / self.capital_history.len() as f64;
    ...
    pub fn print_pnl_metrics(
        &self,
        symbol: &str,
        pnl_results: &HashMap<String, PnLResult>,
    ) -> HashMap<&str, f64> {
        ...
        table.add_row(vec![
            "Total PnL",
            &format!("${:.2}", total_pnl_with_unrealized),
        ]);
        table.add_row(vec![
            "Fill rate",
            &format!("{:.2}%", costs.get("fill_rate").unwrap_or(&0.0) * 100.0),
        ]);
        table.add_row(vec![
            "Buy trades",
            &costs.get("buy_trades").unwrap_or(&0.0).to_string(),
        ]);
        table.add_row(vec![
            "Sell trades",
            &costs.get("sell_trades").unwrap_or(&0.0).to_string(),
        ]);
    ...
    pub fn print_capital_metrics(&self, capital_metrics: &HashMap<String, CapitalMetrics>) {
        for (symbol, metrics) in capital_metrics {
            ...
            table.add_row(vec![
                ...
                &format!("${:.2}", metrics.max_required_capital),
            ]);
            table.add_row(vec![
                "Max drawdown",
            ...
            table.add_row(vec![
                ...
                &format!("${:.2}", metrics.peak_margin_requirement),
            ]);
            table.add_row(vec![
                "Max open positions value",
                &format!("${:.2}", metrics.max_open_positions_value),
            ]);
            table.add_row(vec![
                "Average capital utilization",
                &format!("${:.2}", metrics.average_capital_utilization),
            ]);
            table.add_row(vec![
                "Max unrealized loss",
                &format!("${:.2}", metrics.max_unrealized_loss),
            ]);
    ...
    pub fn to_console(
        ...
        symbol: &str,
        pnl_results: &HashMap<String, PnLResult>,
        capital_metrics: &HashMap<String, CapitalMetrics>,
    ) {
